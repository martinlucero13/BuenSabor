...
export default function EleccionMarcaVino() {
    ...
    async function handleLoad() {
        ...
        try {
            const { data: getRubroProductosMenu } = await api.post('rubroProductos/getRubroProductosMenu')
                ...
                <Modal.Body>
                    <div>
                        ...
                        <select onChange={handleRetiro} defaultValue=''>
                            <option value="" disabled>Seleccione lugar de retiro</option>
                            <option value='1'>Retiro Por Sucursal</option>
                            <option value='2'>Llevar a Domicilio</option>
                        </select>
                        <button disabled={disabled} className={disabled ? 'disabledButton' : 'buttonRetira'} onClick={() => setOpen(false)}>Seleccionar</button>
                    </div>
                </Modal.Body>
            </Modal>
            {
