...
export default function StockProductosFormIngredientes({
  handleShow,
  dataRow,
}) {
  ...
  async function handleLoad() {
    try {
      const { data: rubroList } = await api.get(
        "stockProductos/getRubrosIngredientes"
      );
  ...
  async function handleLoadSubrubro() {
    try {
      const { data: subrubroList } = await api.get(
        "stockProductos/getIngredientes",
        { params: { rubro: formData.rubro } }
      );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { data: editStockProductos } = await api.post(
        "stockProductos/createIngredientesProductos",
        { fromData: dataToSend }
      );
        ...
        <div>
          ...
          <section>
            ...
            >
              ...
              {subrubros.map((rubro) => (
                <option
                  key={rubro.idArticuloInsumo}
                  value={rubro.idArticuloInsumo}
                >
                  {rubro.denominacion}
                </option>
              ))}
