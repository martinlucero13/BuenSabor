...
export default function StockProductosEdit({ handleShow, datoRow }) {
  const INITIAL_STATE = {
    tiempoEstimadoCocina: datoRow ? datoRow.tiempoEstimadoCocina : 0,
    denominacion: datoRow ? datoRow.denominacion : "",
    precioVenta: datoRow ? datoRow.precioVenta : 0,
    descripcion: datoRow ? datoRow.descripcion : "",
    rubro: datoRow ? datoRow.idRubro : "",
    receta: datoRow ? datoRow.receta : "",
  };
  ...
  async function handleLoad() {
    try {
      const { data: rubroList } = await api.get(
        "rubroProductos/getRubroProductos"
      );
  ...
  async function handleSubmit() {
    ...
    try {
      const dataToSend = {
        ...formData,
        idData: datoRow.idArticuloManufacturado,
      };
      const { data: editStockProductos } = await api.post(
        "stockProductos/editStockProductos",
        { fromData: dataToSend }
      );
      ...
      <main>
        <div>
          ...
          <section className={transition ? "element-hidden" : "element"}>
            ...
            <input
              onChange={handleChangeForm}
              value={formData.tiempoEstimadoCocina}
              type="number"
