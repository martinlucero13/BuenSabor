...
export default function RubroIngredientesEdit({ handleShow, datoRow }) {
    const INITIAL_STATE = {
        denominacion: datoRow ? datoRow.denominacion : '',
        habilitado: datoRow ? datoRow.habilitado : '',
    }
    ...
    async function handleSubmit() {
        ...
        try {
            const dataToSend = {
                ...formData,
                idData: datoRow.idRubro
            }
            const { data: editRubroProductos } = await api.post('rubroProductos/editRubroProductos', { fromData: dataToSend })
            if (editRubroProductos.statusCode === 200) {
            ...
            <main>
                <div>
                    ...
                    <section className={transition ? "element-hidden" : "element"}>
                        ...
                        <input placeholder="producto" maxLength={200} onChange={handleChangeForm} value={formData.denominacion} type="text" name="denominacion" />
                    </section>
                    <section>
                        <label>Estado</label>
