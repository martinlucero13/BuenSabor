...
export default function EstadisticasProductosTable() {
    ...
    async function handleLoad() {
        ...
        setLoading(true)
        try {
            const { data: getEstadisticasProductos } = await api.post('estadisticas/getEstadisticasProductos', { dateDesde: formData.dateDesde, dateHasta: formData.dateHasta })
            setDataTable(getEstadisticasProductos.data)
        } catch (error) {
            setDataTable([])
        } finally {
            setLoading(false)
    ...
    const handleOnExport = () => {
        try {
            if (dataTable.length > 0) {
                ...
                XLSX.utils.book_append_sheet(wb, ws, "EstadisticasProductos");
                XLSX.writeFile(wb, "EstadisticasProductos.xlsx");
            } else {
    ...
    function dataFormatExcel(dataTable) {
        return dataTable.map((data) => {
            return {
                Cantidad: data.cantArt,
                Producto: data.nomPro,
                Rubro: data.nomRub,
            };
        });
    }
